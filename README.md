...
The app automatically reads Render's `PORT` variable and binds to the public
network interface when the `RENDER` environment variable is present. You do
not need to add port-related environment variables yourself.

Important: Render's filesystem is temporary unless you attach a persistent
disk. Without one, accounts and image URLs can disappear after a restart or
redeploy. Mount a persistent disk and set `SIG_ROTATOR_DB` to a path on that
disk, such as `/var/data/sig-rotator.db`.
